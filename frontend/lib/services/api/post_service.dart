...
class PostService {
  ...
  Future<List<dynamic>> getTrendingPosts({String? token, int limit = 20, int offset = 0}) async {
    try {
      final queryParams = {'limit': limit.toString(), 'offset': offset.toString()};
      final response = await _apiClient.get(
        ApiEndpoints.postsTrending, // Ensure this is defined correctly in ApiEndpoints
        token: token,
  ...
  Future<List<dynamic>> getFollowingFeed({
    ...
    try {
      ...
      final response = await _apiClient.get(
        ApiEndpoints.feedFollowing, // Use the correct endpoint from ApiEndpoints
        token: token,
        queryParams: queryParams,
      );
      return response as List<dynamic>? ?? [];

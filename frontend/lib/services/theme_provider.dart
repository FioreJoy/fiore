...
class ThemeProvider with ChangeNotifier {
  ...
  ThemeMode _themeMode = ThemeMode.system; // Still used for system/light/dark toggle
  String _currentThemeName = 'Fiore Dark'; // Default to one of your new palette names
  ...
  ThemeProvider() : _currentThemeData = buildThemeFromPalette(fioreDarkPalette) { // Initial theme
    print("ThemeProvider: Initializing...");
    _loadSavedTheme();
  }

  ThemeMode get themeMode => _themeMode; // For system/light/dark toggle
  ThemeData get currentTheme => _currentThemeData; // The actual ThemeData to apply
  ...
  Future<void> _loadSavedTheme() async {
    ...
    final savedThemeModeString = prefs.getString('themeMode'); // Legacy key for ThemeMode

    print("ThemeProvider: Loading saved theme. Name: $savedThemeName, Mode: $savedThemeModeString");

    if (savedThemeName != null && _availablePalettes.containsKey(savedThemeName)) {
      ...
      _themeMode = selectedPalette.brightness == Brightness.dark ? ThemeMode.dark : ThemeMode.light;
      ...
      if (savedThemeModeString == 'light') {
        ...
        _currentThemeName = fioreDarkPalette.name; // Default if system and no name
        _currentThemeData = buildThemeFromPalette(fioreDarkPalette);
      }
      print("ThemeProvider: Loaded theme based on legacy ThemeMode: $_themeMode, applied: $_currentThemeName");
    } else {
      ...
      _currentThemeData = buildThemeFromPalette(selectedPalette);
       _themeMode = selectedPalette.brightness == Brightness.dark ? ThemeMode.dark : ThemeMode.light;
      print("ThemeProvider: No saved theme, defaulted to '$_currentThemeName'.");
    }
  ...
  Future<void> toggleSimpleTheme(ThemeMode mode) async {
    if (_themeMode == mode && _currentThemeName.startsWith("Fiore")) return; // No change needed if already on simple toggle
    ...
    if (mode == ThemeMode.light) {
      ...
      // A better way would be to have a getter in ThemeProvider that resolves the actual theme for system.
       _currentThemeName = fioreDarkPalette.name; // Default to dark for system if no better logic
       _currentThemeData = buildThemeFromPalette(fioreDarkPalette); // This might be incorrect until next build
       print("ThemeProvider: Switched to System mode. Actual theme depends on platform brightness.");
    }
    
    await prefs.setString(_themeNamePrefKey, _currentThemeName); // Save the chosen Fiore Light/Dark
    await prefs.setString('themeMode', themeModeStr); // Save the ThemeMode itself

    print("ThemeProvider: Simple theme toggled to $_themeMode, applied '$_currentThemeName'");
    notifyListeners();
  }
  ...
  ThemeData getSystemResolvedTheme(BuildContext context) {
    if (_themeMode == ThemeMode.system) {
      ...
      final systemPalette = platformBrightness == Brightness.dark ? fioreDarkPalette : fioreLightPalette;

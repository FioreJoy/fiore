...
//       try {
//         // --- FIX IS HERE: Correct method name and use named parameters ---
//         final response = await apiService.signUp(
//             name: _name,
//             username: _username,
//             email: _email,
//             password: _password,
//             gender: _gender ?? 'PreferNotSay', // Provide default if null
//             currentLocation: _location ?? '(0,0)', // Provide default if null
//             college: _college ?? '', // Provide default if null
//             interests: _selectedInterests,
//             image: _profileImage // Pass the File object
//           // token not needed for signup
//         );
//         // Assuming signup returns token and user_id
//         final String token = response['token'];
//         final int userId = response['user_id'];
//
//         // Login the user after successful signup
//         await authProvider.loginSuccess(token, userId.toString()); // Use loginSuccess
//
//         if (mounted) {
...
import 'dart:typed_data'; // <--- Keep for _profileImageData if needed
import 'package:flutter/material.dart';
...
import '../theme/theme_constants.dart';
import '../main.dart'; // <--- ADDED for MainNavigationScreen
...
class _SignUpFormScreenState extends State<SignUpFormScreen> {
  ...
  // Use TextEditingControllers for better state management
  final _nameController = TextEditingController();
  final _usernameController = TextEditingController();
  final _emailController = TextEditingController();
  final _passwordController = TextEditingController();
  ...
  final _locationController = TextEditingController(); // For "(lon,lat)" string

  String? _selectedGender; // Store selected gender
  File? _profileImageFile; // Store the selected image file <--- CHANGED from Uint8List
  List<String> _selectedInterests = []; // Store selected interests

  // Example interest options - replace with your actual logic if needed
  final List<String> _allInterests = ['Gaming', 'Tech', 'Music', 'Sports', 'Art', 'Reading', 'Travel', 'Food'];
  ...
  Future<void> _pickImage() async {
    ...
    final pickedFile = await picker.pickImage(source: ImageSource.gallery, imageQuality: 75); // Compress slightly
    if (pickedFile != null) {
      setState(() {
        _profileImageFile = File(pickedFile.path); // Store File object
      });
    }
  }
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      // --- FIXES START HERE ---
      try {
        // Use text from controllers
        final response = await apiService.signUp(
            name: _nameController.text,
            username: _usernameController.text,
            email: _emailController.text,
            password: _passwordController.text, // Use the first password controller
            gender: _selectedGender ?? 'PreferNotSay', // Provide default if null
            // Default location or use controller value
            currentLocation: _locationController.text.isNotEmpty ? _locationController.text : '(0,0)',
            college: _collegeController.text, // Use college controller
            interests: _selectedInterests, // Use the list of selected interests
        ...
        final String token = response['token'];
        final int userId = response['user_id'];
        // Optionally use the image_url from response if needed immediately

        // Use the correct AuthProvider method (assuming it's like this)
        await authProvider.loginSuccess(token, userId.toString(), response['image_url']); // Pass token, user ID, and optional image URL

        if (mounted) {
          // Use the correct screen name
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => const MainNavigationScreen()),
          );
        }
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Signup failed: ${e.toString()}'),
              backgroundColor: ThemeConstants.errorColor, // Use theme error color
            ),
          );
        }
      }
  ...
  // Helper widget for interest chips (example)
          ...
          onSelected: (bool selected) {
            setState(() {
              if (selected) {
                ...
                _selectedInterests.removeWhere((String name) => name == interest);
              }
            });
          },
            ...
            child: Column(
              ...
              children: <Widget>[
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        backgroundImage: _profileImageFile != null
                            ? FileImage(_profileImageFile!) as ImageProvider // Use FileImage
                            : const NetworkImage(AppConstants.defaultAvatar), // Default fallback
                      ),
                      Material(
                ...
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration( // Match CustomTextField style if possible
                      labelText: 'Gender',
                      prefixIcon: const Icon(Icons.wc_outlined),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(8.0)),
                      contentPadding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 15.0)
                  ),
                  items: ['Male', 'Female', 'Other', 'PreferNotSay'].map((String value) {
                  ...
                  validator: (value) {
                    if (value != null && value.isNotEmpty) {
                      ...
                      if (!RegExp(pattern).hasMatch(value)) {
                        return 'Invalid format. Use (lon,lat) or leave blank';
                      }
                    }
                ...
                // Interests
                Text('Select Your Interests (Optional)', style: Theme.of(context).textTheme.titleMedium),

...
class LocationService {
  ...
  final Map<String, String> _headers = {
    'User-Agent':
        'ConnectionsApp/1.0 (Fiore Contact: divanshthebest@gmail.com)' // Replace placeholders
  };

  Future<Map<String, dynamic>?> getAddressFromCoordinates(
      double latitude, double longitude) async {
    final url = Uri.parse(
        '$_nominatimBase/reverse?format=jsonv2&lat=$latitude&lon=$longitude&accept-language=en');
  ...
  Future<List<Map<String, dynamic>>> searchPlaces(String query) async {
    ...
    final url = Uri.parse(
        '$_photonBase/?q=${Uri.encodeComponent(query)}&limit=5&lang=en');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map<String, dynamic> && data['features'] is List) {
          ...
          return List<Map<String, dynamic>>.from(
              features.whereType<Map<String, dynamic>>());
        } else {
  ...
  Map<String, dynamic>? formatPhotonResult(Map<String, dynamic> feature) {
    try {
      ...
      final double? lon = (lonRaw is num)
          ? lonRaw.toDouble()
      ...
      return {
        'display_name':
            displayName.isEmpty ? 'Unnamed Location (Photon)' : displayName,
        'latitude': lat,
        'longitude': lon,
      };
    } catch (e, stackTrace) {

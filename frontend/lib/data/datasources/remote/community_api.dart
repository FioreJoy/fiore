...
class CommunityService {
  ...
  Future<Map<String, dynamic>> getCommunityDetails(int communityId,
      {String? token}) async {
    try {
      final response = await _apiClient.get(
  ...
  Future<Map<String, dynamic>> joinCommunity(
      int communityId, String token) async {
    try {
  ...
  Future<Map<String, dynamic>> addPostToCommunity({
    ...
    try {
      final response = await _apiClient.post(
        ApiEndpoints.communityPostLink(communityId, postId),
        token: token,
      );
      return response as Map<String, dynamic>;
    } catch (e) {
  ...
  Future<List<dynamic>> getCommunityMembers(int communityId,
      {String? token, int limit = 50, int offset = 0}) async {
    try {
      // Assuming an endpoint like /communities/{id}/members
      // Update ApiEndpoints.dart if this new endpoint is added
      final String endpoint =
          '${ApiEndpoints.communityBaseId(communityId)}/members';
      final queryParams = {

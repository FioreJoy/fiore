...
class MediaItem { // Simple model for media items in a message
  final String id;
  final String? url;
  ...
  factory MediaItem.fromJson(Map<String, dynamic> json) {
    return MediaItem(
      id: json['id']?.toString() ?? '', // Backend usually sends int, convert to string
...
class MessageModel {
  ...
  factory MessageModel.fromJson(Map<String, dynamic> json) {
    ...
    return MessageModel(
      id: json['id']?.toString() ?? '',
      senderId: json['user_id']?.toString() ?? json['sender_id']?.toString() ?? '', // Handle potential key differences
  ...
  Map<String, dynamic> toJson() => {
    ...
    'media': media?.map((item) => { // Basic serialization for media
      'id': item.id,
      'url': item.url,

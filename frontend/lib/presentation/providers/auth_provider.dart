...
class AuthProvider with ChangeNotifier {
  ...
  bool _isLoading =
      true; // True initially, set to false after auto-login attempt
  ...
  final StreamController<AuthProvider> _userStateController =
      StreamController<AuthProvider>.broadcast();
  ...
  Future<void> _tryAutoLogin() async {
    ...
    try {
      ...
      if (_isLoading || initialAuthStatus != isAuthenticated) {
        ...
        print(
            "AuthProvider: Auto-login attempt finished. isLoading: false, isAuthenticated: $isAuthenticated");
      } else {
  ...
  Future<void> loginSuccess(
      String token, String userId, String? imageUrl) async {
    bool stateChanged = (_token != token ||
        _userId != userId ||
    ...
    try {
      ...
      print(
          "AuthProvider: Login Success - User: $userId, Token & Image URL persisted.");
    } catch (e) {
  ...
  Future<void> logout() async {
    ...
    try {
      ...
      print(
          "AuthProvider: Error clearing stored credentials during logout: $e");
    }
    if (stateChanged) {
      notifyListeners();
      if (!_userStateController.isClosed) _userStateController.add(this);
  ...
  Future<void> updateUserImageUrl(String? newImageUrl) async {
    if (_userImageUrl == newImageUrl && !_isLoading)
      return; // No change needed and not loading
    ...
    try {
      ...
      print(
          "AuthProvider: Updated user image URL to '$newImageUrl' and persisted.");
    } catch (e) {

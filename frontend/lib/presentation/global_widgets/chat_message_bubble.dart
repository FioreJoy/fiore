...
// Data Layer Imports
import '../../data/models/message_model.dart'; // For MediaItem and MessageModel definitions
...
class ChatMessageBubble extends StatelessWidget {
  ...
  String _formatBytes(int? bytes, {int decimals = 1}) {
    /* ... Unchanged ... */ if (bytes == null || bytes <= 0) return "0 B";
  ...
  Widget _buildAvatar(BuildContext context) {
    /* ... Unchanged, uses AppConstants which has correct path already ... */
            ...
            onTap: () async {
              ...
              if (await canLaunchUrl(uri))
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(
                        'Could not open: ${mediaItem.originalFilename ?? 'attachment'}')));
            },
  ...
  Widget build(BuildContext context) {
    /* ... UI uses theme and MessageModel, core/app constants for fallback image ... */
                        ...
                        style: TextStyle(
                            color: isMe
                                ...
                                : (isDark
                                    ...
                                    : Colors.black87),
                            fontSize: 15)),
                  _buildMediaAttachmentsWidget(context),
                  Padding(
                    padding: EdgeInsets.only(
                        top: (message.content.isNotEmpty ||
                                (message.media?.isNotEmpty ?? false))
                            ? 5.0
                            : 0),
  ...
  String _formatTime(DateTime timestamp) {
    /* ... Unchanged ... */ final now = DateTime.now();
    ...
    if (messageDate == today)
      ...
      return '${DateFormat.MMMd().format(timestamp)} ${timestamp.hour.toString().padLeft(2, '0')}:${timestamp.minute.toString().padLeft(2, '0')}';
  }
}
